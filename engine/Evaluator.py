...
class Evaluator:
    ...
    def evaluate_intermediate_hyb_rankings(self):
        ...
        intermediate_rankings = self.__get_intermediate_rankings()
    ...
    def __get_intermediate_rankings(self):
        ...
        intermediate_rankings = []  # each item is a list representing each fold iteration
                                    # these lists contain the rankings generated by each bootstrap

        for fold_iteration in range(self.dm.num_folds):
            
            bs_rankings = []
            for bootstrap in range(self.dm.num_bootstraps):
                ranking_path = self.dm.results_path + "fold_" + str(fold_iteration+1) + "/" + \
                    "bootstrap_" + str(bootstrap) + "/"
                file = ranking_path + AGGREGATED_RANKING_FILE_NAME
                ranking = self.dm.load_RDS(file)
                ranking = self.dm.r_to_pandas(ranking)
                bs_rankings.append(ranking)

            intermediate_rankings.append(bs_rankings)

        return intermediate_rankings
