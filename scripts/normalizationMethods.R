...
LOGArrayOnly <- function(array.dt, zero.to.one = TRUE){
    ...
    if (zero.to.one) {
        array.dt <- zero_to_one_transform(array.dt)
    }
    return(array.dt)
}
...
LOGSeqOnly <- function(seq.dt, zero.to.one = TRUE){
    ...
    #  message("\tZero to one transformation...\n")
    if (zero.to.one) {
        log.dt <- zero_to_one_transform(log.dt)
    }
    return(log.dt)
}
...
QNSingleDT <- function(dt, zero.to.one = TRUE){
    ...
    #  message("\tZero to one transformation...\n")
    if (zero.to.one) {
        qn.dt <- zero_to_one_transform(qn.dt)
    }
    return(qn.dt)
}
...
QNSingleWithRef <- function(ref.dt, targ.dt, zero.to.one = TRUE){
    ...
    #  message("\tZero to one transformation...\n")
    if (zero.to.one) {
        qn.targ <- zero_to_one_transform(qn.targ)
    }
    return(qn.targ) 
}
...
TDMSingleWithRef <- function(ref.dt, targ.dt, zero.to.one = TRUE){
    ...
    #  message("\tZero to one transformation...\n")
    if (zero.to.one) {
        tdm.targ <- zero_to_one_transform(tdm.targ)
    }
    return(tdm.targ)
}
...
QNProcessing <- function(array.dt, seq.dt, zero.to.one = TRUE){
    ...
    if (zero.to.one) {
        array.dt <- zero_to_one_transform(array.dt)
        qn.seq <- zero_to_one_transform(qn.seq)
    }
    #  message("\tConcatenation...\n")
...
NPNProcessing <- function(array.dt, seq.dt, zero.to.one = TRUE){
    ...
    #  message("\tZero to one transformation...\n")
    if (zero.to.one) {
        npn.cat <- zero_to_one_transform(npn.cat)
    }
    return(npn.cat)
}
...
TDMProcessing <- function(array.dt, seq.dt, zero.to.one = TRUE){
    ...
    if (zero.to.one) {
        array.dt <- zero_to_one_transform(array.dt)
        tdm.seq <- zero_to_one_transform(tdm.seq)
    }
    #  message("\tConcatenation...\n")
...
LOGProcessing <- function(array.dt, seq.dt, zero.to.one = TRUE){
    ...
    if (zero.to.one) {
        array.dt <- zero_to_one_transform(array.dt)
        log.seq <- zero_to_one_transform(log.seq)
    }
    #  message("\tConcatenation...\n")
